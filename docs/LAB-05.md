# LAB-05 — Desplegar y comprobar la aplicación en Google Cloud
...
La imagen contiene un proceso `uvicorn`, `servicio/`, las dependencias y dos archivos de referencia: `crm/pedidos.txt` y `crm/politicas.txt`. La aplicación lee esos dos TXT desde la imagen. La validación docente completó este patrón con una revisión privada, GCS y Langfuse; cada estudiante conserva evidencia de su propia ejecución.
...
Ubícate en la carpeta `labs/` del proyecto descargado. Prepara una carpeta que contenga únicamente los archivos requeridos por el `Dockerfile`:
...
Si no puedes usar una carpeta temporal, guarda este `.gcloudignore` dentro de `labs/`:
...
gcloud run deploy "$SERVICE" \
  ...
  --memory=512Mi \
  --concurrency=1 \
  --set-env-vars="OPENAI_BASE_URL=https://openrouter.ai/api/v1,OPENAI_MODEL=openai/gpt-4o-mini,CLIENTE_DEMO=C-001,FECHA_ESCENARIO=2026-09-08,CRM_BUCKET=${RECORDS_BUCKET},LANGFUSE_ENABLED=false,LANGFUSE_BASE_URL=https://cloud.langfuse.com" \
