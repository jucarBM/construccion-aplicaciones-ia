...
async def _conversar(entrada: SolicitudChat) -> RespuestaChat:
...
async def conversar(entrada: SolicitudChat) -> RespuestaChat:
    datos = {"pedido_id": entrada.pedido_id, "motivo": entrada.motivo,
             "confirmado": entrada.confirmacion is not None}
    with observacion("chat", datos) as span:
        resultado = await _conversar(entrada)
